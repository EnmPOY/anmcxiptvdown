...
# Enmpoy krallığının anahtarları
...
def hf_yukle_ve_link_al(dosya_yolu, anime_adi):
    ...
    print(f"{Renk.MAVI}[*] Buluta, Enmpoy krallığına mermi gibi ateşleniyor: {dosya_adi}...{Renk.SIFIRLA}")
    try:
        api.upload_file(
        ...
        return f"https://huggingface.co/datasets/{REPO_ID}/resolve/main/{hedef_yol_kodlu}"
    except Exception as e:
        print(f"{Renk.KIRMIZI}[-] Hasiktir, yükleme patladı: {e}{Renk.SIFIRLA}")
        return None
...
def m3u_txt_kaydet_ve_yukle(anime_adi, sezon, bolum, raw_link):
    ...
    with open(dosya_adi, "a", encoding="utf-8") as f:
        if ilk_mi: f.write("#EXTM3U\n") 
        f.write(f"#EXTINF:-1, {anime_adi} - S{sezon:02d} E{bolum:02d}\n{raw_link}\n")
    
    print(f"{Renk.YESIL}[+] {anime_adi} S{sezon} E{bolum} M3U listesine zımbalandı!{Renk.SIFIRLA}")
    
    try:
        HfApi().upload_file(path_or_fileobj=dosya_adi, path_in_repo=dosya_adi, repo_id=REPO_ID, repo_type="dataset", token=HF_TOKEN)
        print(f"{Renk.SARI}[*] Güncel IPTV listesi buluta yedeklendi.{Renk.SIFIRLA}")
    except:
        print(f"{Renk.KIRMIZI}[-] Liste senkronize edilirken hata çıktı.{Renk.SIFIRLA}")

def anigexis_saf_guc_motoru(url_sablonu, anime_adi, sezon, baslangic, bitis):
    print(f"\n{Renk.YESIL}===================================================={Renk.SIFIRLA}")
    print(f"{Renk.YESIL}[*] G63'ÜN SAF GÜÇ ODAKLI ANİME SÖMÜRÜCÜSÜ AKTİF! (PROXY YOK){Renk.SIFIRLA}")
    print(f"{Renk.YESIL}===================================================={Renk.SIFIRLA}\n")

    with sync_playwright() as p:
        # Aracı yok, proxy yok, sadece saf hız!
        ...
        for bolum_no in range(baslangic, bitis + 1):
            ...
            # JS ile Webdriver (Bot) izlerini siliyoruz
            ...
            try:
                print(f"{Renk.SARI}    [*] Hedefe doğrudan, maskesiz sızılıyor...{Renk.SIFIRLA}")
                ...
                # FARK ETMEZ MODU: Kim çevirmişse çevirmiş, ilkine tıkla geç!
                fansub_kartlari[0].click()
                ...
                print(f"{Renk.KIRMIZI}    [-] Siteyle kapışırken yara aldık: {hata}{Renk.SIFIRLA}")
            finally:
                baglam.close()
                ...
                try:
                    ...
                    if inen_dosyalar:
                        ...
                        if hf_direkt_link:
                            ...
                            print(f"{Renk.SARI}    [*] İndirme bitti, GitHub'ın yerel çöpleri temizlendi.{Renk.SIFIRLA}")
                except Exception as e:
                    print(f"{Renk.KIRMIZI}    [-] yt-dlp patladı: {e}{Renk.SIFIRLA}")
            else:
...
if __name__ == "__main__":
    ...
    # Hedeflerini buradan belirle, gerisini bota bırak
    anigexis_saf_guc_motoru(url_sablonu=sablon, anime_adi="Naruto", sezon=1, baslangic=1, bitis=220)
